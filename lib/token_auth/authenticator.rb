module TokenAuth
  module Authenticator
    ...
    def ensure_entity_authenticated!
      raise Unauthorized.new(I18n.t("token_auth.authenticator.errors.auth_faild", default: "Authenitcation faild")) unless @current_entity.present?
    end
    ...
    module ClassMethods
      def acts_as_token_authenticator_for(authenticable_class, options = {})
        ...
        before_filter :"authenticate_#{authenticable_class_underscored}_by_token", options
        before_filter :"ensure_#{authenticable_class_underscored}_authenticated!", options
        ...
        class_eval <<-AUTHENTICATOR, __FILE__, __LINE__ + 1
          ...
          def current_#{authenticable_class_underscored}
            @current_entity
          end
        AUTHENTICATOR
      end
